/**
 * session.js — Cookie & localStorage session persistence for Ellucian Banner Student
...
async function loadSession(page) {
  ...
  if (session.localStorage && Object.keys(session.localStorage).length > 0) {
    ...
    const url = new URL(session.url || process.env.ELLUCIAN_BANNER_URL || 'https://localhost');
